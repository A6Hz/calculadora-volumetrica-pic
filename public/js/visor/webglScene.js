...
var renderer = window.WebGLRenderingContext ? 
    new THREE.WebGLRenderer({ antialias: true, preserveDrawingBuffer: true, alpha: false } ) : 
    new THREE.CanvasRenderer();

var container, camera, light3dbin, light1, controls = null, box, axis, planes;
var offset = 0;
...
function initScene() {    
    ...
    renderer.shadowMap.type = THREE.PCFSoftShadowMap;    console.log(container.offsetWidth)
    renderer.setSize( container.offsetWidth - 20 , container.offsetHeight - offset );
    ...
    light3dbin = new THREE.DirectionalLight( 'rgb(255,255,255)', 1);
    light3dbin.shadow.mapSize.width = 1024; 
    light3dbin.shadow.mapSize.height = 1024;
	  scene.add( light3dbin );
            
    window.addEventListener( 'resize', onWindowResize, false );   
    
    //THREEx.Screenshot.bindKey(renderer);    
    document.getElementById("sshot").onclick = (function takeScreenShot() { 
        //THREEx.Screenshot.call(renderer); }

        var canvas = document.querySelectorAll("#webgl-container canvas");
...
function setLight(w, h, d) {
  light3dbin.lookAt( scene.position );
  light3dbin.position.copy( camera.position );
}
...
function onWindowResize() {    
    ...
    camera.updateProjectionMatrix();
    renderer.setSize(container.offsetWidth - offset -50 , container.offsetHeight-offset);
